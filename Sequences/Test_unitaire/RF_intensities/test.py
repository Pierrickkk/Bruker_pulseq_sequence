...
dwellbruk=6e-3/(len(sinc_complex) * system.rf_raster_time)

sinc_bruk_inv = pp.make_arbitrary_rf(
    no_signal_scaling=True,
    signal=sinc_complex,
    dwell=dwellbruk*1e-6,
    flip_angle=10* math.pi / 180,
    delay=system.rf_dead_time
)
sinc_bruk_inv.signal = sinc_bruk_inv.signal/np.max(np.abs(sinc_bruk_inv.signal))

sinc_bruk_inv.signal = sinc_bruk_inv.signal * np.sqrt((((10/90)*(1e-3/6e-3)*1/0.16895))**2 *(250)**2)  # scale the signal to match the peak of the bruker sech.inv pulse; note the sqrt(1000) from above. The 0.16895 is the integral of the original sinc.inv pulse, which can be calculated by integrating the original RF shape (after converting to B1 in Tesla and multiplying by gamma) over time.
...
sinc10_stretched_bruk_inv = pp.make_arbitrary_rf(
    no_signal_scaling=False,
    signal=sinc_stretched,
    flip_angle=10* math.pi / 180,
    delay=system.rf_dead_time
)
sinc10_stretched_bruk_inv.signal = sinc10_stretched_bruk_inv.signal/np.max(np.abs(sinc10_stretched_bruk_inv.signal))

sinc10_stretched_bruk_inv.signal = sinc10_stretched_bruk_inv.signal * np.sqrt((((10/90)*(1e-3/6e-3)*1/0.16895))**2 *(250)**2)  # scale the signal to match the peak of the bruker sech.inv pulse; note the sqrt(1000) from above. The 0.16895 is the integral of the original sinc.inv pulse, which can be calculated by integrating the original RF shape (after converting to B1 in Tesla and multiplying by gamma) over time.

sinc20_stretched_bruk_inv = pp.make_arbitrary_rf(
    no_signal_scaling=False,
    signal=sinc_stretched,
    flip_angle=20* math.pi / 180,
    delay=system.rf_dead_time
)
sinc20_stretched_bruk_inv.signal = sinc20_stretched_bruk_inv.signal/np.max(np.abs(sinc20_stretched_bruk_inv.signal))

sinc20_stretched_bruk_inv.signal = sinc20_stretched_bruk_inv.signal * np.sqrt((((20/90)*(1e-3/6e-3)*1/0.16895))**2 *(250)**2)  # scale the signal to match the peak of the bruker sech.inv pulse; note the sqrt(1000) from above. The 0.16895 is the integral of the original sinc.inv pulse, which can be calculated by integrating the original RF shape (after converting to B1 in Tesla and multiplying by gamma) over time.

sinc30_stretched_bruk_inv = pp.make_arbitrary_rf(
    no_signal_scaling=False,
    signal=sinc_stretched,
    flip_angle=30* math.pi / 180,
    delay=system.rf_dead_time
)
sinc30_stretched_bruk_inv.signal = sinc30_stretched_bruk_inv.signal/np.max(np.abs(sinc30_stretched_bruk_inv.signal))

sinc30_stretched_bruk_inv.signal = sinc30_stretched_bruk_inv.signal * np.sqrt((((30/90)*(1e-3/6e-3)*1/0.16895))**2 *(250)**2)  # scale the signal to match the peak of the bruker sech.inv pulse; note the sqrt(1000) from above. The 0.16895 is the integral of the original sinc.inv pulse, which can be calculated by integrating the original RF shape (after converting to B1 in Tesla and multiplying by gamma) over time.
sinc40_stretched_bruk_inv = pp.make_arbitrary_rf(
    no_signal_scaling=False,
    signal=sinc_stretched,
    flip_angle=40* math.pi / 180,
    delay=system.rf_dead_time
)
sinc40_stretched_bruk_inv.signal = sinc40_stretched_bruk_inv.signal/np.max(np.abs(sinc40_stretched_bruk_inv.signal))

sinc40_stretched_bruk_inv.signal = sinc40_stretched_bruk_inv.signal * np.sqrt((((40/90)*(1e-3/6e-3)*1/0.16895))**2 *(250)**2)  # scale the signal to match the peak of the bruker sech.inv pulse; note the sqrt(1000) from above. The 0.16895 is the integral of the original sinc.inv pulse, which can be calculated by integrating the original RF shape (after converting to B1 in Tesla and multiplying by gamma) over time.
...
rf90_bruk_stretched_inv = pp.make_arbitrary_rf(
    no_signal_scaling=False,
    signal=rf_stretched,
    flip_angle=90* math.pi / 180,
    delay=system.rf_dead_time
...
rf90_bruk_stretched_inv.signal = rf90_bruk_stretched_inv.signal/np.max(np.abs(rf90_bruk_stretched_inv.signal))

rf90_bruk_stretched_inv.signal = rf90_bruk_stretched_inv.signal * np.sqrt((((90/90)*(1e-3/6e-3)*1/0.106428))**2 *(250)**2)  # scale the signal to match the peak of the bruker sech.inv pulse; note the sqrt(1000) from above. The 0.16895 is the integral of the original sinc.inv pulse, which can be calculated by integrating the original RF shape (after converting to B1 in Tesla and multiplying by gamma) over time.
...
for i in range(1):
    ...
    seq.add_block(pp.make_delay(0.1e-3))
    seq.add_block(sinc10_stretched_bruk_inv, gz,pp.make_delay(7e-3))
    seq.add_block(pp.make_delay(0.1e-3))
    seq.add_block(sinc20_stretched_bruk_inv, gz,pp.make_delay(7e-3))
    seq.add_block(pp.make_delay(0.1e-3))
    seq.add_block(sinc30_stretched_bruk_inv, gz,pp.make_delay(7e-3))
    seq.add_block(pp.make_delay(0.1e-3))
    seq.add_block(sinc40_stretched_bruk_inv, gz,pp.make_delay(7e-3))
    seq.add_block(pp.make_delay(0.1e-3))
    ...
    seq.add_block(pp.make_delay(0.1e-3))
    seq.add_block(rf90_bruk_stretched_inv, gz,pp.make_delay(7e-3))
    seq.add_block(pp.make_delay(0.1e-3))
