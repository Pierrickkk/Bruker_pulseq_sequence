...
output_path = "/workspace_QMRI/PROJECTS_DATA/2026_RECH_bruker_pulseq/pypulseq/python/Test_unitaire/multiple_ADC/output"
filename = "AT_GRE_multiple_ADC.seq"
...
adc2 = pp.make_adc(num_samples=Nx//2, duration=gx.flat_time, delay=gx.rise_time, system=system)
...
for s in range(n_slices):
    ...
    for i in range(Ny):
        rf.phase_offset = rf_phase / 180 * np.pi
        adc.phase_offset = rf_phase / 180 * np.pi
        adc2.phase_offset = rf_phase / 180 * np.pi
        rf_inc = divmod(rf_inc + rf_spoiling_inc, 360.0)[1]
        rf_phase = divmod(rf_phase + rf_inc, 360.0)[1]
...
seq.set_definition(key='Name', value='AT_GRE_multiple_ADC')
