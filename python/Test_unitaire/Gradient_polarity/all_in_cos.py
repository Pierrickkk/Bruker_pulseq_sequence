...
adc_trap = pp.make_adc(num_samples=N, duration=gx_trap.flat_time, delay=system.adc_dead_time, system=system)
...
#extended plus
seq.add_block(rf)
seq.add_block(pp.scale_grad(grad = g_tot, scale=1),pp.scale_grad(grad = g_tot_y, scale=0.5),pp.scale_grad(grad = g_tot_z, scale=0.75))
seq.add_block(pp.scale_grad(grad = g_tot, scale=-1),pp.scale_grad(grad = g_tot_y, scale=-0.5),pp.scale_grad(grad = g_tot_z, scale=-0.75))
