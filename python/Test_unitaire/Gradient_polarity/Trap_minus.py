...
def main(plot: bool = False, write_seq: bool = False, seq_filename: str = 'Trap_minus.seq'):
    # ======
    # SETUP
    # ======
    ...
    #Trigger event
    trig=pp.make_trigger(channel='physio2', duration=2000e-6) # duration after
    #trig=pp.makeTriggerPulse('osc0','duration', 4100e-6); # possible channels: 'osc0','osc1','ext1'
    ...
    # Inverting the polarity of gradients
    gx.amplitude *= -1 
    gx_pre.amplitude *= -1
    gz.amplitude *= -1
    gz_reph.amplitude *= -1
    gx_spoil.amplitude *= -1
    gz_spoil.amplitude *= -1
    ...
    for i in range(1):
        ...
        seq.add_block(rf, gz) 
        seq.add_block(trig_out) # add the trigger output pulse after the RF excitation
    ...
    if write_seq:
        ...
        seq.set_definition(key='Name', value='gre minus')

        seq.write("/workspace_QMRI/PROJECTS_DATA/2026_RECH_bruker_pulseq/pypulseq/python/Test_unitaire/Gradient_polarity/ouput" +"/"+ seq_filename)
