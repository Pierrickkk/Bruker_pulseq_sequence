...
if FLAG_TRIG:
    trig_out1 = pp.make_digital_output_pulse('osc0', duration=100e-6, delay=0)  # trigger output pulse
    trig_out2 = pp.make_digital_output_pulse('osc1', duration=100e-6, delay=0)  # trigger output pulse
    trig_out3 = pp.make_digital_output_pulse('ext1', duration=100e-6, delay=0)  # trigger output pulse

    trig_in1 = pp.make_trigger(channel='physio1', duration=2) # trigger input pulse, here with 2s duration
    trig_in2 = pp.make_trigger(channel='physio2', duration=2)
...
if FLAG_TRIG:
    assert np.all(math.ceil(min_TR - trig_out1.duration))
    min_TR = math.ceil((min_TR - trig_out1.duration)/seq.grad_raster_time)*seq.grad_raster_time
...
#seq.add_block(pp.make_delay(time_label), pp.make_label(type='SET', label='LIN', value=0))
if FLAG_TRIG:
    seq.add_block(trig_in1)  # wait for the cardiac trigger 
    seq.add_block(trig_out1)
rf.phase_offset = rf_phase / 180 * np.pi
adc.phase_offset = rf_phase / 180 * np.pi
rf_inc = divmod(rf_inc + rf_spoiling_inc, 360.0)[1]
rf_phase = divmod(rf_phase + rf_inc, 360.0)[1]
...
#seq.add_block(pp.make_delay(time_label), pp.make_label(type='SET', label='LIN', value=1))
if FLAG_TRIG:
    seq.add_block(trig_in1)  # wait for the cardiac trigger 
    seq.add_block(trig_out2)

#Excitation
seq.add_block(rf, gz)
seq.add_block(gx_pre, pp.scale_grad(grad=gy_pre,scale=scale_area[2]), gz_reph)
seq.add_block(b_delay_TE)
seq.add_block(gx, adc)

seq.add_block(gx_spoil, pp.scale_grad(grad=gy_pre,scale=-scale_area[2]), gz_spoil)
seq.add_block(b_delay_TR)



#seq.add_block(pp.make_delay(time_label), pp.make_label(type='SET', label='LIN', value=2))
if FLAG_TRIG:
    seq.add_block(trig_in1)  # wait for the cardiac trigger 
    seq.add_block(trig_out2, pp.make_label(type='SET', label='LIN', value=2))

#Excitation
seq.add_block(rf, gz)
seq.add_block(gx_pre, pp.scale_grad(grad=gy_pre,scale=scale_area[3]), gz_reph)
seq.add_block(b_delay_TE)
seq.add_block(gx, adc)

seq.add_block(gx_spoil, pp.scale_grad(grad=gy_pre,scale=-scale_area[3]), gz_spoil)
seq.add_block(b_delay_TR)

#seq.add_block(pp.make_delay(time_label), pp.make_label(type='SET', label='LIN', value=3),rf,gz)
#if FLAG_TRIG:
#    seq.add_block(trig_in1)  # wait for the cardiac trigger 
#    seq.add_block(trig_out2)
...
if FLAG_WRITE_SEQ:
    output_path = "/workspace_QMRI/PROJECTS_DATA/2026_RECH_bruker_pulseq/pypulseq/python/Test_unitaire/Extension/output"
    seq_type = "_trig1_ttl122_nooverlap_nextid"
