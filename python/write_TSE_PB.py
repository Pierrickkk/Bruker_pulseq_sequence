...
def main(
    ...
    *,
    fov: float | tuple[float, float] = 20e-3,
    n_x: int = 128,
    n_y: int = 128,
    n_echo: int = 8,
    n_slices: int = 1,
    rf_flip_deg: int = 180,
    slice_thickness: float = 0.3e-3,
    te: float = 20e-3,
    tr: float = 1000e-3,
):
    ...
    gs1 = pp.make_extended_trapezoid(channel='z', times=gs1_times, amplitudes=gs1_amp,system=system)

    gs2_times = np.array([0, gs_ex.flat_time])
    gs2_amp = np.array([gs_ex.amplitude, gs_ex.amplitude])
    gs2 = pp.make_extended_trapezoid(channel='z', times=gs2_times, amplitudes=gs2_amp, system=system)
    ...
    gs3 = pp.make_extended_trapezoid(channel='z', times=gs3_times, amplitudes=gs3_amp, system=system)

    gs4_times = np.array([0, gs_ref.flat_time])
    gs4_amp = np.array([gs_ref.amplitude, gs_ref.amplitude])
    gs4 = pp.make_extended_trapezoid(channel='z', times=gs4_times, amplitudes=gs4_amp, system=system)
    ...
    gs5 = pp.make_extended_trapezoid(channel='z', times=gs5_times, amplitudes=gs5_amp, system=system)
    ...
    gs7 = pp.make_extended_trapezoid(channel='z', times=gs7_times, amplitudes=gs7_amp, system=system)
    ...
    gr5 = pp.make_extended_trapezoid(channel='x', times=gr5_times, amplitudes=gr5_amp, system=system)

    gr6_times = np.array([0, readout_time])
    gr6_amp = np.array([gr_acq.amplitude, gr_acq.amplitude])
    gr6 = pp.make_extended_trapezoid(channel='x', times=gr6_times, amplitudes=gr6_amp, system=system)
    ...
    gr7 = pp.make_extended_trapezoid(channel='x', times=gr7_times, amplitudes=gr7_amp, system=system)
...
if __name__ == '__main__':
    main(fov=30e-3, slice_thickness=0.3e-3)